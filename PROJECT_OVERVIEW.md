...
**结构化元数据**：额外建了 `paper_details`（中英文标题/DOI/作者/期刊/年份/关键词，以及区分论文/教材/专利的 `doc_type` 和专利的申请号/申请日/法律状态）、`paper_assets`（图表+图题+页码）、`paper_references`（每篇论文自己引用的参考文献逐条记录）三张表。提取策略是"免费优先"——先从 MinerU 产出的 `content_list.json` 本地解析（正则抓 DOI、按 block 类型取图表和参考文献），解析不出来的字段才调用大模型补，省了不少 token。
...
**专利**：库里除了论文和教材还混着专利，它们不需要单独入口——跟论文走同一条流水线，`enrich` 阶段按扉页的 INID 码（`(21)申请号` `(22)申请日` `(72)发明人` 这类全球通用的著录项目代码）自动认出来，命中 ≥2 个标记才算（只提过一次 "United States Patent" 的论文不会被误判），认出来后 `doc_type='patent'`，发明人/申请人/申请日直接从 INID 码取，跳过 LLM 补缺。这里有个关键区分：**"状态"其实是两件事**——出版阶段（专利号末尾种别码 `A`=申请公布、`B`=发明授权、`U`=实用新型）和保护期（申请日 + 20/10/15 年）是印在 PDF 上的事实，本地现算、零成本、不落库；而驳回、撤回、欠年费失效、无效宣告这些**真正的法律状态既不在 PDF 上、又会随时间变化**，只能人工核实，落 `legal_status` + `status_checked_at` 两列（必须带核实日期，否则数据会悄悄烂掉）。没核实过就明说"未核实"，绝不拿种别码 `B` 冒充"现在仍然有效"。命令是 `review.py patents [--rescan [--apply]] [--set 专利号 --status 状态]`，检索用 `--corpus patent` 限定。
...
`mcp_server.py` 把整个文献库检索和综述撰写能力包装成标准 MCP 协议 server，Claude Code / Claude Desktop / Cherry Studio 这些客户端配置一下就能在对话里直接调用，不用自己敲命令行。开放了 7 个工具：查库状态、语义检索（可按论文/教材/专利限定语料）、查单篇文献元数据（专利号也能查）、查专利台账、生成大纲、后台起一篇综述（因为耗时 7-30 分钟，用的是"秒回 job_id + 轮询"的异步模式）、查生成进度。

走 stdio 协议，所以除了本机直连，也能让别的设备通过 SSH 接进来共用同一个库（经一条反向隧道落到跑着文献库的那台机器上）。7 个工具对论文库全是**只读**的，唯一的写入是把生成的综述 md/docx 落盘。专利法律状态尤其刻意只读——那是要对外负责的结论，不能让模型自己判断完就写进生产库，只能走 CLI 人工录入。
...
入库流水线和综述生成器都已跑通，有真实文献库在用：**179 篇/本已完整入库（论文 157、教材 20、专利 2），向量索引覆盖 167 个文档、12375 个块**，结构化元数据 179 篇（含图表 13928 条、参考文献 5301 条）。MCP 集成、检索精度升级（换 Qwen3 嵌入+重排）、结构化元数据提取、GB 格式引用、章节编号、Word 导出、教材 EPUB 支持、每日定时入库、专利识别与状态台账均已完成，**测试套件 73 个用例全绿**。专利重扫已于 2026-08-04 对生产库执行完毕（迁移前热备份留在 `batch_tracking.db.bak.20260804_101706`），扫出的 2 篇此前一篇被当论文、一篇被当教材；它们的法律状态目前都是"未核实"，待人工去国家知识产权局查证后用 `patents --set` 录入。部署也已去宿主机化——同一份 compose 靠一个 `.env` 就能在开发机、云服务器、别人的电脑上跑，并已在阿里云实机验证过一轮。
