...
入库流水线和综述生成器都已跑通，有真实文献库在用：**179 篇/本已完整入库（论文 158、教材 21），向量索引覆盖 167 个文档、12375 个块**，结构化元数据 179 篇（含图表 13928 条、参考文献 5301 条）。MCP 集成、检索精度升级（换 Qwen3 嵌入+重排）、结构化元数据提取、GB 格式引用、章节编号、Word 导出、教材 EPUB 支持、每日定时入库、专利识别与状态台账均已完成，**测试套件 73 个用例全绿**。（专利识别代码已就绪并在生产库副本上验证过——扫出 2 篇此前被当成论文的专利——但尚未对生产库执行 `patents --rescan --apply`，所以上面的分类计数里它们还算在论文里。）部署也已去宿主机化——同一份 compose 靠一个 `.env` 就能在开发机、云服务器、别人的电脑上跑，并已在阿里云实机验证过一轮。
