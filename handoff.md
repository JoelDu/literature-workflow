...
- **chunk 级断点续跑**：每 4 块落一次库；`md_hash` 未变时直接续算未嵌的块，**绝不调 `replace_doc_chunks`**（它头一句 `DELETE FROM review_chunks` 会删掉上一晚的成果）；全部块都有向量才 `mark_embedded`。一本 300 页教材约 117 块 ≈ 7.8 小时，10 小时窗口内跑得完；两本连排就必然跨夜。
- 速度基准:**约 60 秒/块，与块长基本无关，一夜(10h) ≈ 600 块**。别用"字符/小时"估时，会跑偏数倍。
